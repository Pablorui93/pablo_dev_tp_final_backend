...
class WorkspaceController {
    ...
    static async delete(request, response){
        try{
            const { workspace_selected, user } = request
            const workspace_id = workspace_selected._id
            
            // La validación de rol 'admin' ya se hace en el router con workspaceMiddleware(['admin'])

            await WorkspaceService.deleteById(workspace_id, user.id)
            ...
            if(error.status){
                ...
                console.error(
                    'ERROR AL ELIMINAR EL WORKSPACE', error
                )
                return response.status(500).json({
                    ok: false,
                    message: 'Error interno del servidor',
