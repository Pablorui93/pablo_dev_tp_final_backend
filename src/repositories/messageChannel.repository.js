...
class MessagesChannelRepository {
    ...
    static async deleteAllByChannelId(channel_id) {
        try {

            const messages_to_delete = await MessageChannel.find({ channel_id: channel_id }).select('_id');

            for (const msg of messages_to_delete) {
                await MessagesChannelRepository.deleteById(msg._id);
            }

            return { deletedCount: messages_to_delete.length };
        }
        catch (error) {
            console.error('[SERVER ERROR]: no se pudieron eliminar los mensajes del canal ' + channel_id, error);
            throw error
        }
    }
}
