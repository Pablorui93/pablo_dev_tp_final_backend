...
class ChannelRepository {
    ...
    static async getByIdAndWorkspaceId(workspace_id, channel_id) {
        ...
        try {
            const result = await Channel.findByIdAndDelete({ id_workspace: workspace_id })
            return result
        }
        catch (error) {
            console.error('[SERVER ERROR]: no se pudieron eliminar los canales del workspace ' + workspace_id, error);
            throw error
        }
    }
}
